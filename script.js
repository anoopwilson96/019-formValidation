...
submitButtonJS.addEventListener('click', function(event) {
  // Call fillAll function to perform validation and display errors
    fillAll();
    });
    
    function fillAll() {
      // ... your existing validation logic here (checking empty fields, password complexity, etc.)
      if (!nameInputJS.value || !emailInputJS.value || !passwordInput1JS.value || !passwordInput2JS ) {
        // Display error message: 
        nameErrorJS.innerHTML  = "Name cannot be blank." ;
        emailErrorJS.innerHTML = "Email address cannot be blank.";
        passwordError1JS.innerHTML = "Password must contain at least one uppercase letter, one lowercase letter, one special character, one digit, and be at least 8 characters long. ";
        passwordError2JS.innerHTML = "Password must contain at least one uppercase letter, one lowercase letter, one special character, one digit, and be at least 8 characters long. ";
